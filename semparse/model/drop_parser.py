...
class DropSemanticParser(Model):
    """
    ...
    def __init__(self,
        ...
        # numbers, strings, relations
        self._num_entity_types = 3  # TODO(mattg): get this in a more principled way somehow?
    ...
    @staticmethod
    def _get_neighbor_indices(worlds: List[DropWorld],
                              num_entities: int,
        ...
        num_neighbors = 0
        for world in worlds:
            for entity in world.knowledge_graph.entities:
                if len(world.knowledge_graph.neighbors[entity]) > num_neighbors:
                    num_neighbors = len(world.knowledge_graph.neighbors[entity])

        batch_neighbors = []
        no_entities_have_neighbors = True
        for world in worlds:
            # Each batch instance has its own world, which has a corresponding table.
            entities = world.knowledge_graph.entities
            entity2index = {entity: i for i, entity in enumerate(entities)}
            entity2neighbors = world.knowledge_graph.neighbors
            neighbor_indexes = []
    ...
    @staticmethod
    def _get_type_vector(worlds: List[DropWorld],
                         num_entities: int,
        ...
        for batch_index, world in enumerate(worlds):
            types = []
            for entity_index, entity in enumerate(world.knowledge_graph.entities):
                # We need numbers to be first, then relation, then strings, because our entities are
                # going to be sorted.  We do a split by type and then a merge later, and it relies on
                # this sorting.
                if entity.startswith('relation:'):
                    entity_type = 1
