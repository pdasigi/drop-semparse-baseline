...
from semparse.model.drop_parser import DropSemanticParser

@Model.register("drop_mml_parser")
class DropMmlParser(DropSemanticParser):
    """
    A ``DropMmlParser`` is a :class:`DropParser` which is trained to
    ...
    variable free language from ``semparse.language.drop_type_declaration``.
    ...
    def forward(self,  # type: ignore
        ...
        world : ``List[DropWorld]``
            ...
            how ``MetadataField`` works, this gets passed to us as a ``List[DropWorld]``,
        actions : ``List[List[ProductionRuleArray]]``
